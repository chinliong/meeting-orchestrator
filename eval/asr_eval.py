...
when no key is configured. Gemini is retained as a configurable backend but is not the default.

{_wrap('''Gemini wins the benchmark: 10.9% word error rate against 17.5% for the strongest
Whisper model, at a fifth of the wall-clock time. It is not the shipped default, because
deploying it exposed a failure this table cannot show. Its audio endpoint sheds load in bursts,
returning HTTP 503 for stretches long enough to exhaust a foreground request - a six-minute
upload failed after nine rejected attempts, and the fallback never ran. The same clip was
rejected and then accepted seconds later, from two different networks, while text requests to
the same models answered in about a second throughout, so the cause is upstream capacity for
audio rather than payload size, quota or the calling host.''')}

{_wrap('''Whisper therefore ships. It is slower and roughly 5 points less accurate, and it
never fails: no key, no network and no quota for the local model, and a stable hosted endpoint
otherwise. Accuracy that is intermittently unavailable is worth less to a user than accuracy
that is merely lower, and a benchmark run from one machine on one afternoon cannot see the
difference. Gemini remains configurable for anyone whose deployment does not hit the same
capacity pressure.''')}
