...
_Extraction accuracy: [evaluation-report.md](evaluation-report.md). Subtask generation: [subtask-evaluation-report.md](subtask-evaluation-report.md)._

## Decision

{_wrap('''Uploads are transcribed by **Deepgram Nova-3**, the most accurate of the configurations
...
makes ES2010a a useful end-to-end case.''')}

{_wrap('''The meeting assigns work to three roles but names only one person: "The ID, which is
who? Okay. You're going to think about the working design... And UID. The technical functions
design... And the marketing person, Courtney, is going to do the user requirements
specification." Claude Sonnet returned seven items and filled in owners for all three roles,
attributing the working design to "Vanilla" and the technical functions to "Amber" - two names
the transcript never connects to those roles, one of them a transcription error. Gemini Flash
returned the four items the meeting actually commits to and left both unnamed roles
unassigned.''')}

{_wrap('''The confidence score separated the two cases. Both invented attributions fell
