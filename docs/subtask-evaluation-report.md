...
_Generated by `python -m eval.subtask_eval --write-report` on 2026-09-14._
...
- Judge model: `claude-sonnet-4-6` (held fixed across both configurations)
...
**Subtask checklists are generated by Gemini Flash.** Over 5 runs per configuration it scores 4.786 out of 5 against Claude Sonnet's 4.822, a gap of 0.036 that an exact paired permutation test over the per-task means does not separate (p = 0.652), so the two are indistinguishable on quality. The choice rests on cost rather than quality: Gemini is roughly a tenth the price per token and keeps the system on a single provider.

The run-to-run spread inside each configuration (Gemini Flash 4.69-4.88, sd 0.081; Claude Sonnet 4.67-4.94, sd 0.102) is wider than the 0.036 between them, and Claude Sonnet leads only nominally. What does differ is shape rather than standard: Claude Sonnet produced 5.36 subtasks per task against Gemini Flash's 4.74, scoring higher on coverage where Gemini Flash scores higher on non-redundancy.

The judge was Claude in both configurations. That means Claude assessed its own output in one and a competitor's in the other, an asymmetry that favours Claude - and Claude still does not separate from Gemini, so the tie is not an artefact of a partial judge.
