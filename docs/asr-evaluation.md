...
_Generated by `python -m eval.asr_eval` on 2026-09-14. Re-run to refresh._
...
represent them. But the limit is set by segmentation, not model capacity: scaling Whisper from
...
but on the implemented configuration it reaches the board only as a name nobody is assigned to,
not as an owner.
