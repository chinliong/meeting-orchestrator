...
_Generated by `python -m eval.asr_eval` on 2026-09-19. Re-run to refresh._
...
both times. The three Whisper rows are not separable on ES2008a. On ES2010a hosted turbo is
ahead of the other two, so the two hosted Whisper models swap order between the meetings.
...
That refines the limit rather than removing it. Roughly a tenth of the reference words in
ES2008a are spoken while someone else is talking, and a single mixed channel plus a linear
