...
export default function StatsBar({ tasks }: Props) {
  ...
  const cards = [
    { label: "To Do", value: todo, dot: "bg-brand" },
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {cards.map((c) => (
        <div
          key={c.label}
          className="relative overflow-hidden rounded-xl border border-slate-200/80 bg-white px-4 py-3"
        >
          ...
          <p
            className={`relative mt-1 font-display text-3xl font-bold tracking-tight ${
